...
#' @param select.cues Optional selection of outcomes to limit the number 
...
plotOutcomeWeights <- function(wmlist, outcome, select.cues=NULL, 
	init.value=0, add.labels=TRUE, add=FALSE,
	...){
	dat <- getWeightsByOutcome(wmlist=wmlist, outcome=outcome, select.cues=select.cues, init.value=init.value)
	...
	if(!'main' %in% names(par)){
		par[['main']] <- sprintf("Outcome: \"%s\"", outcome)
	}
