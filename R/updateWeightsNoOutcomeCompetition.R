#' Function implementing the Rescorla-Wagner learning equations without 
...
updateWeightsNoOutcomeCompetition <- function(cur.cues, cur.outcomes, 
	...
	wm[cur.cues,] =  wm[cur.cues,] + matrix( rep(lr * (Lambda - Vtotal) * Lambda, length(cur.cues)), nrow=length(cur.cues), byrow=TRUE )
	return(wm)
}
